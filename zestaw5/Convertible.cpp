...
/* tym razem korzystamy ze zwykłychych przeciążonychonych funkcji */
static one test(U) ;
static two test(...);
static T makeT();
...
main() {
  ...
  cerr<<Is_convertible<vector<double>::value_type,double >::same_type<<endl;

  cerr<<Is_convertible<vector<double>::iterator,double *>::same_type<<endl;
}